...
\begin{frame}[fragile]{Les déclencheurs, ou  \textit{triggers}}
...
\begin{blocgauche}
\alert{Notez}\,: les variables \sqlkw{old} et \sqlkw{new}, représentant le nuplet
...
\begin{redbullet}
  \item Un \textit{trigger} est déclenché par  une \alert{insertion, destruction ou modification} 
   \item La \alert{condition} est testée, et si elle n'est pas satisfaite, l'exécution du \textit{trigger} s'arrête
  \item L’\alert{action} est effectuée  à l’aide du langage procédural  (PL/SQL)
\end{redbullet}
...
La création du \textit{trigger} suivant  \alert{bloque} le programme qui 
l'exécute. 
\vfill
