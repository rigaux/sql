---
title: 'SQL, récapitulatif'
---

Ce chapitre présente les compléments du langage d\'interrogation SQL (la
partie dite *Langage de Manipulation de Données* ou LMD) dans le cadre
d\'un récapitulatif. Ces compléments présentent peu de difficulté dans
la mesure où la véritable complexité réside d\'une part dans
l\'interprétation des requêtes complexes qui font parfois appel à des
logiques sophistiquées et d\'autre part dans la multiplicité des
variantes syntaxiques qui peut parfois troubler.
...
d\'interrogation, l\'un déclaratif et l\'autre procédural. Les requêtes
se comprennent soit via leur équivalent en formules logiques, soit en
les considérant comme des opérations ensemblistes.

Dans ce chapitre nous utilisons systématiquement l\'approche
déclarative. Vous pouvez les reformuler sous leur forme ensembliste si
vous le souhaitez.
...
-   [Diapositives: le bloc \`select-from-where
    \<http://sql.bdpedia.fr/files/slblocsql.pdf\>]{.title-ref}\_
-   [Vidéo sur le bloc
    ]{.title-ref}[select-from-where]{.title-ref}<https://mdcvideos.cnam.fr/videos/?video=MEDIA180917190541813>
:::

Dans cette session, nous étudions les compléments à la forme de base
d\'une requête SQL, que nous appelons *bloc*, résumée ainsi:
...
Parmi les quatre clauses `select`, `form`, `where` et `order by`, les
...
`where` et le \* désigne tous les attributs.
...
> -   la clause `where` exprime un ensemble de conditions sur la source:
>     seuls les nuplets pour lesquels ces conditions sont satisfaites
>     sont conservés;
> -   enfin la clause `select` construit un nuplet-résultat grâce à une
>     liste d\'expressions appliquées aux nuplets de la source ayant
>     passé le filtre du `where`.

La clause `from`
----------------

L\'espace de recherche est défini dans la clause `from` par une ou
...
`from` peut également être *résultat* d\'une autre requête. On parlera
...
Il n\'aura pas échappé au lecteur attentif que le résultat est lui-même
une table (calculée et non basée). Pourquoi ne pourrait-on pas
interroger cette table calculée comme une autre? C\'est possible en SQL
...
Comme nous l\'avons vu, cette approche est de nature algébrique: on
manipule dans le `from` des ensembles, stockés (les tables) ou calculés
(obtenus par des requêtes). C\'est une syntaxe en plus pour dire la même
chose, donc on peut très bien se passer de la seconde formulation. Il
est plus intéressant de prolonger cette idée d\'interroger une relation
*calculée* en donnant définivement un nom à la requête qui sélectionne
l\'immeuble. En SQL cela s\'appelle une *vue*. On crée une vue dans un
schéma avec la commande `create view`.
...
Les vues sont traitées en détail dans le chapitre consacré aux schémas
relationnels.
:::

L\'interprétation du `from` est indépendante de l\'origine des tables:
...
de recherche avec le `from`. La première est la forme déclarative dans
...
(voir chapitre `chap-calcul`{.interpreted-text role="ref"}) à laquelle
on peut affecter tous les nuplets de la table. Les variables peuvent
être explicitement nommées avec la mot-clé `as` (elles s\'appellent `i`
et `a` dans la requête ci-dessus). On peut aussi omettre le `as`, dans
ce cas le nom de la variable est (implcitement) le nom de la table.

``` {.sql}
select * from Immeuble, Appart
```

Un cas où le `as` est obligatoire est l\'auto-jointure: on veut désigner
deux nuplets de la même table. Exemple: on veut les paires
d\'appartement du même immeuble.
...
En l\'absence du `as` et de l\'utilisation du nom de la variable comme
...
Cette formulation revient à définir une table virtuelle (appelons-là
*Tfrom*) qui tient lieu d\'espace de recherche par la suite.
L\'affichage ci-dessus nous montre quel est l\'espace de recherche
*Tfrom* de la requête précédente.
...
La clause de jointure `join` définit un espace de recherche constitué
...
On obtient le résultat suivant.
  ...
  id     nom              adresse          id     surface   niveau   idImmeuble   no
  ------ ---------------- ---------------- ------ --------- -------- ------------ ------
  1      Koudalou         3 rue des        100    150       14       1            1
                          Martyrs                                                 

  1      Koudalou         3 rue des        101    50        15       1            34
                          Martyrs                                                 

  1      Koudalou         3 rue des        102    200       2        1            51
                          Martyrs                                                 

  1      Koudalou         3 rue des        103    50        5        1            52
                          Martyrs                                                 

  1      Koudalou         3 rue des        104    75        3        1            43
                          Martyrs                                                 

  2      Barabas          2 allée du Grand 200    150       0        2            1
                          Turc                                                    

  2      Barabas          2 allée du Grand 201    250       1        2            1
                          Turc                                                    

  2      Barabas          2 allée du Grand 202    250       2        2            2
                          Turc                                                    
  --------------------------------------------------------------------------------------

L\'obligation d\'encadrer les expressions algébriques quand on on en
combine plusieurs (par exemple jointure entre trois tables ou plus) les
rend difficilement lisibles. C\'est une des raisons qui poussent à s\'en
tenir à la version déclarative de SQL.

Dernière précision au sujet du `from`: l\'ordre dans lequel on énumère
...
nuplets désignés par la clause `from`. Ces conditions suivent en général
la syntaxe `expr1 [not]` $\Theta$ `expr2`, où `expr1` et `expr2` sont
...
manquantes: on dit qu\'elles sont à `null`. Le `null` *n\'est pas une
...
Les valeurs à `null` sont une source de problème, car elles rendent
parfois le résultat des requêtes difficile à comprendre. Mieux vaut les
éviter si c\'est possible.
:::

Il est impossible de déterminer quoi que ce soit à partir d\'une valeur
à `null`. Dans le cas des comparaisons, la présence d\'un `null` renvoie
...
des prénoms à `null`. La requête suivante devrait ramener tous les
nuplets.
...
Mais la présence d\'un `null` empêche l\'inclusion du nuplet
...
`is not null`). La requête ci-dessous ramène tous les nuplets de la
...
Finalement, une fois obtenus les nuplets du `from` qui satisfont le
`where` on crée à partir de ces nuplets le résultat final avec les
...
Le résultat montre que l\'une des valeurs est à `null`. Logiquement
toute opération appliquée à un `null` renvoie un `null` en sortie
...
cette valeur à `null`. Dans ce cas, il faut utiliser une fonction
(spécifique à chaque système)à qui remplace la valeur à `null` par une
...
Une \"expression\" dans la clause `select` désigne ici, comme dans tout
...
Les attributs `surface` et `niveau` proviennent de *Appart* alors que 18
...
combiner des données réparties dans plusieurs tables. Nous avons déjà
étudié en détail la conception et l\'expression des jointures. On va se
contenter ici de montrer quelques exemples en forme de récapitulatif,
sur notre base d\'immeubles.
...
SQL. Il est recommandé de se limiter à la forme de base donnée
ci-dessous qui est plus facile à interpréter et se généralise à un
nombre de tables quelconques.
:::

### Jointure interne

Prenons l\'exemple d\'une requête cherchant la surface et le niveau de
l\'appartement de M. Barnabé Simplet.
...
apparaissent en double soit ici `id` qui peut désigner l\'identifiant de
...
clé étrangère correspondante de l\'autre. Mais rien n\'empêche
d\'exprimer des conditions de jointure sur n\'importe quel attribut et
pas seulement sur ceux qui sont des clés.

Imaginons que l\'on veuille trouver les appartements d\'un même immeuble
qui ont la même surface. On veut associer un nuplet de *Appart* à un
autr nuplet de *Appart* avec les conditions suivantes:
...
L\'approche déclarative d\'expression des jointures est une manière tout
à fait recommandable de procéder surtout pour les débutants SQL. Elle
permet de se ramener toujours à la même méthode d\'interprétation et
consolide la compréhension des principes d\'interrogation d\'une base
relationnelle.
...
Qu\'est-ce qu\'une jointure externe? Effectuons la requête qui affiche
tous les appartements avec leur occupant.
...
tous ses nuplets, même ceux qui ne trouvent pas de correspondant dans la
table de droite, seront prises en compte. Les nuplets de la table de
droite sont en revanche optionnels.
...
et *Personne*. Le mot-clé `left` est optionnel.
...
Notez les deux attributs `prénom` et `nom` à `null` pour l\'appartement
34.
...
SQL renvoie les nuplets du résultat sans se soucier de la présence de
...
Parmi les requêtes ci-dessous, laquelle nous donnera la liste des
logements où on n'a pas informé le lieu

A)  `I`{.interpreted-text role="eqt"} select \* from Logement where lieu
    = \'\'
B)  `C`{.interpreted-text role="eqt"} select \* from Logement where lieu
    is null
C)  `I`{.interpreted-text role="eqt"} select \* from Logement where lieu
    = null
:::
...
Voici trois manières de chercher des livres dont l\'année est connue.
Sont-elles toutes correctes et équivalentes?
...
A)  `I`{.interpreted-text role="eqt"} faux
B)  `C`{.interpreted-text role="eqt"} vrai
:::
...
On peut demander explicitement le tri du résultat sur une ou plusieurs
expressions avec la clause

A)  `I`{.interpreted-text role="eqt"} `left outer by`
B)  `C`{.interpreted-text role="eqt"} `order by`
C)  `I`{.interpreted-text role="eqt"} `desc by`
:::
...
A)  `C`{.interpreted-text role="eqt"} faux
B)  `I`{.interpreted-text role="eqt"} vrai
:::
...
A)  `I`{.interpreted-text role="eqt"} Le résultat de la jointure externe
    entre R et S est inclus dans le résultat de la jointure classique
B)  `C`{.interpreted-text role="eqt"} Le résultat de la jointure
    classique entre R et S est inclus dans le résultat de la jointure
    externe
C)  `I`{.interpreted-text role="eqt"} La jointure externe et la jointure
    classique entre R et S ont une intersection non vide mais ne sont
...
Pas de support vidéo pour cette session qui ne fait que récapituler les
différentes syntaxes équivalentes pour exprimer une même requête. Ne
vous laissez pas troubler par la multiplicité des options offertes pqr
SQL. En choisissant un dialecte et un seul (vous avez compris que je
vous recommande la partie déclarative, logique de SQL) vous pourrez tout
exprimer sans avoir à vous poser des questions sans fin. Vos requêtes
n\'en seront que plus cohérentes et lisibles.
:::

Dans tout ce qui précède, les requêtes étaient \"à plat\", avec un seul
bloc `select-from-where`. SQL est assez riche (ou assez inutilement
...
Reprenons l\'exemple de la requête trouvant la surface et le niveau de
l\'appartement de M. Simplet. On peut l\'exprimer avec une requête
imbriquée de deux manières. La première est la forme déclarative
classique.
...
une jointure classique (exercice). Parfois l\'expression avec requête
imbriquée peut s\'avérer plus naturelle. Supposons que l\'on cherche les
immeubles dans lesquels on trouve un appartement de 50 m2. Voici
...
`chap-calcul`{.interpreted-text role="ref"}).

Ces conditions sont les suivantes:

> -   `exists R`: renvoie `true` si *R* n\'est pas vide `false` sinon.
> -   *t* `in R` où est un nuplet dont le type (le nombre et le type des
>     attributs) est celui de *R*: renvoie `true` si *t* appartient à
>     *R* `false` sinon.
> -   *v* *cmp* `any R` où *cmp* est un comparateur SQL (*\<* *\>* *=*
>     etc.): renvoie `true` si la comparaison avec *au moins un* des
>     nuplets de la table *R* renvoie `true`.
> -   *v* *cmp* `all R` où *cmp* est un comparateur SQL (*\<* *\>* *=*
>     etc.): renvoie `true` si la comparaison avec *tous* les nuplets de
>     la table *R* renvoie `true`.
...
Le `all` peut se réécrire avec une négation puisque si une propriété est
*toujours* vraie il n\'existe pas de cas où elle est fausse. La requête
ci-dessous applique le `all` pour chercher le niveau le plus élevé de
l\'immeuble 1.
...
Le `all` exprime une comparaison qui vaut pour *toutes* les nuplets
ramenés par la requête imbriquée. La formulation avec `any` s\'écrit:
...
Attention aux valeurs à `null` dans ce genre de situation: toute
comparaison avec une de ces valeurs renvoie `unknown` et cela peut
entraîner l\'échec du `all`. Il n\'existe pas d\'expression avec
...
s\'appelle Barnabé Simplet. Il s\'agit assez visiblement d\'une jointure
...
seule table peuvent d\'exprimer avec `exists` ou `in`. Voici quelques
...
dans une sous-requête associée à la clause `exists` peu importent les
attributs du `select` puisque la condition se résume à: cette requête
ramène-t-elle au moins un nuplet ou non? On peut donc systématiquement
...
quel(s) appartement(s) Alice Grincheux est-elle propriétaire et dans
quel immeuble?\" écrite avec plusieurs niveaux.
...
négations. On utilise alors `not in` ou (de manière équivalente)
...
on utilisera typiquement une sous-requête pour prendre le niveau maximal
d\'un immeuble, et on utilisera cet niveau pour sélectionner un ou
...
*qu\'il n\'existe pas* de niveau plus elevé que *a*. On utilise alors
...
-   [Diapositives: agrégats](http://sql.bdpedia.fr/files/slagregats.pdf)
-   [Vidéo sur les
    agrégats](https://mdcvideos.cnam.fr/videos/?video=MEDIA180917190840712)
:::
...
expressions. Ce regroupement est spécifié par la clause `group by`. On
...
7 pour le nombre de noms. En effet, l\'attribut `prénom` est à `null`
...
utiliser `count(*)` ou un attribut déclaré comme `not null`. On peut
...
Le rôle du `group by` est de partitionner le résultat d\'un bloc
`select from where` en fonction d\'un critère (un ou plusieurs
attributs, ou plus généralement une expression sur des attributs). Pour
bien analyser ce qui se passe pendant une requête avec `group by` on
peut décomposer l\'exécution d\'une requête en deux étapes. Prenons
l\'exemple de celle permettant de vérifier que la somme des quote-part
...
(ici `idAppart`), d\'autre part l\'ensemble de nuplets dans lesquelles
...
points-virgule dans la représentation ci-dessous.
  ...
  --------------- --------------------------------------- ---------------
  100             (idPersonne=1 quotePart=33 ;            2
                  idPersonne=5 quotePart=67)              
...
nuplets dans chaque groupe, `max()` donne la valeur maximale d\'un
attribut parmi l\'ensemble des nuplets du groupe, etc. La liste des
fonctions d\'agrégation est donnée ci-dessous:

> -   `count(expression)`, Compte le nombre de nuplets pour lesquels
...
Vous pouvez exécuter ces requêtes sur le site des TP
<http://deptfod.cnam.fr/bd/tp>.
:::

Calculons la surface totale des appartements, groupés par immeuble.
Décomposons: nous avons d\'abord besoin du bloc \"select - from -
where\" avec les identifants d\'immeubles et les surfaces
d\'appartement.
...
On ajoute à cette requête la clause `group by` pour grouper par
...
Revenons un moment à nos voyageurs et à leurs séjours. La requête
ci-dessous doit être claire. Exécutez-la sur le site: on constate qu\'un
voyageur a effectué plusieurs séjours et qu\'un logement a reçu
plusieurs voyageurs.
...
En ajoutant une clause `group by` on produit des statistiques sur le
...
On peut aussi regrouper sur plusieurs attributs. Pour obtenir par
exemple le nombre de séjours effectués par un voyageur dans un même
logement.
...
Moralité: à partir d\'une requête SQL `select - from -- where`, aussi
complexe que nécessaire, vous produisez un résultat (une table). Le
`group by` permet d\'effectuer des regroupements et des agrégations
...
groupes avec la clause `having`. Par exemple, on peut sélectionner les
...
d\'agrégation, par opposition avec la clause `where` qui ne peut
...
La requête ci-dessus pourrait s\'exprimer en utilisant l\'alias pour
éviter d\'avoir à répeter deux fois le `count(*)` (et pour la rendre
plus claire).
...
`group by` soit acceptée dans tous les systèmes.
:::

Quelques exemples pour conclure. Toujours sur la base des voyageurs:
quels voyageurs ont effectué au moins deux séjours ?
...
Voici enfin une requête un peu complexe (sur la base des immeubles)
sélectionnant la surface possédée par chaque copropriétaire pour
l\'immeuble 1. La surface possédée est la somme des surfaces
d\'appartements possédés par un propriétaire, pondérées par leur
quote-part. On regroupe par propriétaire et on trie sur la surface
possédée.
...
Pourquoi peut-on effectuer des *count()*, *sum()* et autres fonctions
d'agrégations sans clause `group by` (premier exemple du cours) ?

A)  `I`{.interpreted-text role="eqt"} Parce que dans ce cas chaque
    nuplet du résultat de la requête avant regroupement est un groupe,
    auquel on applique la fonction
B)  `C`{.interpreted-text role="eqt"} Parce que dans ce cas le résultat
    avant regroupement forme un seul et unique groupe
C)  `I`{.interpreted-text role="eqt"} Parce que la condition de
    sélection dans la clause where définit toujours autant de groupes
    qu'il y a de critères
:::
...
A)  `I`{.interpreted-text role="eqt"} C'est un synonyme du `where`, avec
    lequel elle est interchangeable : on l'utilise pour des raisons de
    lisibilité
B)  `C`{.interpreted-text role="eqt"} Elle permet d'exprimer des
    conditions portant exclusivement sur les groupes, et pas sur les
    nuplets individuels
C)  `I`{.interpreted-text role="eqt"} Elle permet d'éliminer des nuplets
    de certains groupes, une fois ceux-ci constitués
:::
...
A)  `I`{.interpreted-text role="eqt"} Oui : toutes les clauses et
    fonctions d'agrégation sont bien utilisées
B)  `C`{.interpreted-text role="eqt"} Non, car il peut y avoir plusieurs
    noms de logement par type, et le résultat n'est donc pas en première
    forme normale
C)  `I`{.interpreted-text role="eqt"} Non, car il faut appliquer une
    fonction d'agrégation à tous les attributs de la clause `select`
:::
...
pourrait-elle s'écrire sans group by, en remplaçant la clause `having`
...
On cherche les lieux ayant au moins 3 auberges. Quelle est la bonne
requête ?

A)  `I`{.interpreted-text role="eqt"} select lieu from Logement where
    type='Auberge' and count(\*) \> 3
B)  `C`{.interpreted-text role="eqt"} select lieu from Logement where
    type='Auberge' group by lieu having count(\*) \> 3
C)  `I`{.interpreted-text role="eqt"} select lieu from Logement group by
    type having type='Auberge' and count(\*) \> 3
:::
