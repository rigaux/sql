...
\begin{itemize}
  \item Crèche (\textbf{idCrèche}, nom, ville, capacité)
   \item Personne (\textbf{idPersonne}, prénom, nom, néeEn, ville, idPère, idMère)
...
Il faut 
    ...
    \texttt{Personne} (elle peut être à \texttt{null} pour toutes les 
...
\begin{solution}
\begin{itemize}
  \item Non le curseur estimmuable\,: cf. cours
  \item Autant d'inscriptions que de demandes, ce qui ne semble pas normal
  \item La base ne permet de représenter qu'une seule inscription
      pour une année, donc le programme rejettera les insertions 
      dès la seconde tentative.
   \item Il faut sortir de  la boucle dès la première inscription.
\end{itemize}
\end{solution}
