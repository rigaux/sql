...
{Examen 12 juin 2025 -- session 1 -- 3h}%
{Examen en présentiel -- Calculatrices autorisées -- Documents autorisés: 5 pages recto/verso de notes personnelles}%

\textit{Pour chaque réponse, il est recommandé d'expliquer les hypothèses et de détailler
suffisamment les calculs.}
...
Autres statistiques\,:
\begin{itemize}
	\item 2M d'auteurs et 4 auteurs par publication en moyenne\,;
...
\begin{solution}
	L'espace utile dans un bloc est de 7372 octets.\\
	\begin{tabular}{rrrrr}
\textbf{Table} & {\bf Taille tuples} 	& {\bf Nb tuples par bloc} & \textbf{Nb tuples} & {\bf Nb blocs}\\
\hline
Publication&	61 	&	121	&	31 500 000	&	260 330	\\
Journal&	74	&	100	&	26 400	&	264	\\
Citation&	8	&	922	&	553 000 000	&	699 783	\\
Auteur&	108	&	69	&	155 000 000	&	2 246376	\\
\end{tabular}
\end{solution}
...
\begin{itemize}
	...
	\item 1
	\item 1
	\item $155 \times 10^6 /(2\times 10^6) = 77.5$ (soit le nombre moyen de publication par auteur)
\end{itemize}
\end{solution}
...
\begin{solution}
\begin{itemize}
	\item clé : 4+4, et l'adresse occupe  10 octets, soit 18 octets par entrée
	\item On met 410 entrées par bloc, donc l'ordre est 205
	\item $\lceil log_{205}(1,55 \times 10^8)\rceil = 4$
\end{itemize}
\end{solution}
...
\begin{minted}{sql}
SELECT idJournal FROM Journal 
WHERE Titre like '%database%' AND Rang = 'A*';
\end{minted}
	Donner le coût d'évaluation (nombre de lectures de blocs au pire des cas)
   pour chacun des scénarios  suivants :
	\begin{enumerate}
		\item Il n'existe qu'un arbre  B (dense) sur le titre
		\item  Il n'existe qu'un arbre  B (dense) sur le rang, avec la distribution suivante\,:\\
		\begin{tabular}{|c|c|c|c|c|}
			...
			\hline
		\end{tabular}
    \item Il existe qu'un arbre  B sur le rang, mais cette fois il est non dense.
	...
	\begin{solution}
		\begin{enumerate}
			\item L'arbre  ne sert à rien, il faut un parcours séquentiel\,: 264 I/O
			\item On peut utiliser l'arbre  sur le rang, mais il est très peu
      efficace comme le montre le calcul suivant.
      Il existe environ $26\, 400*7,644/100 = 2\,018$ journaux de rang A*,
      donc, au pire, à peu près autant de lectures de blocs, ce qui
	  dépasse largement la lecture séquentielle.

      \item Si l'arbre est non dense, c'est que 
	  le fichier Journal est trié sur le rang.
	  On trouve alors les 2\,018 journaux de rang A*
         séquentiellement, soit $\rceil 2\,018 / 74 \lceil = 27$ blocs
		 (en ignorant le parcours de l'arbre).
...
\begin{enumerate}
   ...
   \item Quelle transaction pourrait effectuer une lecture avec la clause
     \texttt{for update} (ce qui pose un verrou exclusif) 
	 et quel en serait l'effet\,?
\end{enumerate}
