...
{Examen à distance -- Copie en un seul PDF à rendre dans l'espace Moodle}%
...
\section{Jointures et optimisation (9 points)}
...
Quelle taille peut-on estimer pour $R \Join S$\,?
  ...
  \item Je n'ai toujours que 20 blocs en mémoire.
...
\begin{solution}
 \begin{enumerate}
  ...
  \item Dans le pire des cas il y a $200~000 \times 20$ enregistrements
   ...
   200 enregistrements chacune (20 blocs en mémoire). Ensuite je hache $R$ en 25
   fragments également (leur taille m'importe peu). Puis je fais la jointure
   entre chaque paire de fragmentss. Coût (très 	pproximatif) =
...
une clé unique sont \textbf{en gras}).
...
\begin{enumerate}
  \item $T_1 = r_1[x] w_1[x]$, $T_2 = r_2[y] r_2[x] r_2[z] w_2[z]$,
   ...
   \item $r_1[x] r_2[y] w_1[x] r_3[y] (T_2\,\ \rm{attente\ sur }\ r_2[x]) (T_3\ \rm{\ attente\ sur\ }w_3[y])
