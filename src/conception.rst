...
La notion de normalisation a été introduite dans le chapitre :ref:`chap-modrel`. Elle
...
La dépendance fonctionnelle :math:`idPersonne \to prénom, nom`
 ...
 * :math:`idPersonne \to prénom, nom`
 * :math:`idAppart \to surface`
...
.. important:: L'absence de perte d'information est une notion qui est survolée
   ...
   La décomposition d'une table :math:`T`  en 
   plusieurs tables :math:`T_1, T_2, \cdots, T_n` 
   est sans perte d'information quand on peut reconstituer :math:`T`  avec
   des jointures :math:`T_1  \Join T_2  \Join \cdots \Join T_n`.
...
.. admonition:: Algorithme de normalisation

   On part d'un schéma de relation :math:`R`, et on suppose donné
   l'ensemble des dépendances fonctionnelles minimales et directes sur :math:`R`. 
   
   On détermine alors les clés de :math:`R`, et on applique la décomposition:
   
     - Pour chaque DF minimale et directe :math:`X \to A_1, \cdots A_n` 
       on crée une relation  :math:`(X, A_1, \cdots A_n)` de clé :math:`X`
     - Pour chaque clé :math:`C` non représentée dans une des relations
       précédentes, on crée une relation :math:`(C)` de clé :math:`C`.
...
.. important:: Le choix de l'identifiant est un sujet délicat. On peut
...
.. important:: Il faut veiller à ce que  les schémas obtenus soient normalisés. C'est
...
.. csv-table:: 
       :header:  "idRéalisateur",  "titre", "année"
       :widths: 4, 10, 10, 6
...
.. csv-table:: 
   :header:  "idFilm",  "titre", "année", "idRéalisateur"
   :widths: 4, 10, 6, 4
...
.. eqt:: conc1-1
    ...
    Si on me donne un ensemble de dépendances fonctionnelles, est-il possible 
    d’obtenir un schéma relationnel normalisé

    A) :eqt:`I`  Non, il manque des informations 
    #) :eqt:`C`  Oui, en appliquant l’algorithme de normalisation
    #) :eqt:`I` Cela dépend, il faut étudier au cas par cas

.. eqt:: conc1-2

    La décomposition consiste à répartir le contenu d'une table
    initiale en plusieurs tables. Qu'est-ce qu'une décomposition sans perte d'information (SPI)
   
    A) :eqt:`I`  Une décomposition dans laquelle on n'a oublié aucun attribut
    #) :eqt:`C`  Une décomposition dont la table initiale peut être reconstruite par des jointures
    #) :eqt:`I` Une décomposition dans laquelle au moins une des tables contient toutes les clés.


.. eqt:: conc1-3

    Supposons que l'on décide qu'il ne peut y avoir qu'un propriétaire pour un
    appartement, autrement dit une DF :math:`idAppart \to idPersonne`. Quelles affirmations
    sont vraies ?
    
    A) :eqt:`I`  Il devient inutile de décomposer la relation ``Propriétaire``
    #) :eqt:`C`  Nous n'avons plus besoin de table  ``Propriétaire``, car l'identifiant 
       du propriétaire devient un attribut de ``Appart``
    #) :eqt:`I` Cela n'entraîne aucun changement

.. eqt:: conc1-4

    La seconde règle de l'algorithme de normalisation crée une relation pour les clés ``C``
    non représentées. Quelle explication de cette règle vous semble correcte?
    
    A) :eqt:`I` Ces clés représentent des objets ou concepts oubliés par la conception
    #) :eqt:`I`  Parce qu'à chaque clé correspond une relation, et réciproquement
    #) :eqt:`C`  Cette règle permet de représenter un concept, un objet ou un lien 
       sans attribut descriptif, et donc sans dépendance fonctionnelle
...
.. eqt:: conc1-5
    ...
    A) :eqt:`I` On crée deux relations, l’une avec clé A, l’autre avec clé D
    #) :eqt:`I`  On crée une relation avec deux clés primaires
    #) :eqt:`C`  On crée une relation et on choisit A ou D comme clé primaire


.. eqt:: conc1-6

    Que se passe-t-il si on a une relation ABCD et des dépendances A -> B et C -> D ?
    
    A) :eqt:`I` On crée une relation ABCD de clé AC
    #) :eqt:`C` On crée trois relations AB, CD et AC, de clés respectives A, C et AC
    #) :eqt:`I`   On crée deux relations AB et CD, de clés respectives A et C
...
données *Films* (figure :numref:`ea-films`). Sans entrer dans
...
d'implantation. Le schéma de la figure :ref:`ea-films` ne spécifie aucun
...
de données) de *clé*. Reportez-vous au chapitre :ref:`chap-modrel` pour une 
...
.. note:: Certaines méthodes admettent l'introduction de constructions
   plus complexes :
...
.. admonition:: Définition: Type d'entité
...
d'entités :math:`\{e_1, e_2, \ldots e_n\}`, instances d'un même
type :math:`E` est une *extension* de :math:`E`.
...
.. admonition:: Définition: clé
   ...
   Soit :math:`E` un type d'entité et :math:`A` l'ensemble des attributs
   de :math:`E`. Une *clé* de :math:`E` est un sous-ensemble *minimal*
   de :math:`A` permettant d'identifier de manière unique une entité
   parmi n'importe quelle extension de :math:`E`.
...
.. admonition:: Définition: association

    Une association binaire entre les ensembles d'entités
    :math:`E_1` et :math:`E_2` est un ensemble de couples
    :math:`(e_1, e_2)`, avec :math:`e_1 \in E_1` et :math:`e_2 \in E_2`.
...
:numref:`graphe-ea` on remarque que :
...
.. admonition :: Définition: cardinalités

    Soit une association :math:`(E_1, E_2)` entre deux types
    d'entités. La cardinalité de l'association pour  :math:`E_i, i \in \{1, 2\}`,
    est une paire :math:`[min, max]` telle que :

    * Le symbole *max* (cardinalité maximale)
      désigne le nombre *maximal* de fois où une 
      une entité :math:`e_i`  peut intervenir dans l'association.

      En général, ce nombre est 1 (au plus une fois) ou
      :math:`n` (plusieurs fois, nombre indeterminé), noté 
      par le symbole *\**.
...
deux types d'entités :math:`T_A` et :math:`T_B`. Les cardinalités
pour  :math:`T_A` sont placées à l'extrémité du lien
allant de :math:`T_A` à :math:`T_B`  et les cardinalités
pour :math:`T_B` sont l'extrémité du lien
allant de :math:`T_B` à :math:`T_A`.

Pour l'association entre *Réalisateur* et
*Film*, cela donne l'association de la
:numref:`assoc`. Cette association se lit *Un réalisateur
...
exemples est donné dans la  :numref:`graphe-ea2`. On constate tout
...
On obtient le schéma de la  :numref:`assoc2`, avec
...
rôle tenu par l'acteur dans le film (:numref:`assoc2`).
...
.. admonition:: Définition: Clé d'une association

    La clé  d'une association (binaire) entre un type d'entité
    :math:`E_1` et un type d'entité :math:`E_2` est la paire constituée de
    la clé :math:`c_1` de :math:`E_1` et de la clé :math:`c_2` de :math:`E_2`.
...
.. _assoc-internaute:
.. figure:: ./figures-sql/assoc-internaute.png
   :width: 60%
   :align: center

   Association entre *Internaute* et *Film*

Prenons le cas de l'association entre *Internaute* et *Film* (:numref:`assoc-internaute`). Elle
...
.. note:: La réification consiste à transformer en un objet réel et autonome
...
.. _assoc-reifiee:
.. figure:: ./figures-sql/assoc-reifiee.png
   :width: 80%
   :align: center

   réification de l'association entre *Internaute* et *Film*
   
La :numref:`assoc-reifiee` montre le modèle obtenu en transformant l'association *Note*
...
.. note:: Certains outils de conception ne permettent que les associations un-à-plusieurs,
...
.. eqt:: conc2-0
    ...
    A) :eqt:`I`  Une entité de type A peut être associée à plusieurs entités de type B
    #) :eqt:`I`  Une entité de type B peut être associée à une seule entité de type A
    #) :eqt:`C` Une entité de type A peut être associée à une  seule entité de type B

.. eqt:: conc2-1

    L'adresse électronique est choisie comme identifiant pour les internautes. Parmi les inconvénients
    de ce choix ci-dessous, lequel vous semble  le plus pertinent?
   
    A) :eqt:`I`  Il se peut que l'on ne connaisse pas l'adresse d'un internaute
    #) :eqt:`C`  Un internaute peut changer d'adresse électronique, ce qui est compliqué à gérer
    #) :eqt:`I` Un internaute peut ne pas avoir d'adresse électronique


.. eqt:: conc2-2

    Si on choisit comme identifiant un numéro séquentiel, faut-il toujours le nommer ``id`` ?
...
.. eqt:: conc2-3
    ...
    A) :eqt:`I`  Ils sont toujours de type  "chaîne de caractères"
    #) :eqt:`C`  Ils ne prennent qu'une seule valeur pour chaque entité
    #) :eqt:`I`  On connaît toujours leur valeur, pour chaque entité 

.. eqt:: conc2-4

    "Jouer un rôle" est une association plusieurs-plusieurs avec des cardinalités "0..\*". Cela
    signifie:
   
    A) :eqt:`I`  Qu'on peut connaître un rôle sans connaître l'acteur et / ou le film
    #) :eqt:`I`  Qu'il peut y avoir plusieurs rôles joués par le même acteur dans le même film
    #) :eqt:`C`  Qu'un acteur peut jouer plusieurs rôles mais dans des films différents
...
.. eqt:: conc2-5
    ...
    #) :eqt:`C`  Pour une raison X ou Y, ce film a été inséré sans rôle, même
       s'il en a réellement, dans la vraie vie
    #) :eqt:`I`  J'ai fait une erreur de modélisation



.. eqt:: conc2-6

    Supposons que je décide de réifier l'association "Joue un rôle" avec une entité ``Rôle``.
    Quelle différence cela implique-t-il?
   
    A) :eqt:`I`  On ne peut toujours pas avoir un même acteur jouant deux rôles dans un même film
    #) :eqt:`I`  Plusieurs acteurs peuvent jouer le même rôle
    #) :eqt:`C`  Un rôle existe maintenant en tant qu'entité et a donc un identifiant propre
...
par une association classique, comme le montre la  :numref:`faible`.a.
...
dans la  :numref:`faible`.b. La salle est associée au cinéma
...
La présence d'un type d'entité faible :math:`B` associé
à un type d'entité :math:`A` implique également des contraintes fortes
sur les créations, modifications et destructions des instances de
:math:`A` car on doit toujours s'assurer que la contrainte
...
et *Horaire*. On obtient la :numref:`assoc-tern`. 

.. _assoc-tern:
.. figure:: ../figures/assoc-tern.png
   :width: 60%
   :align: center
...
On aurait donc le :math:`n`-uplet *[nomCinéma, noSalle, idFilm, idHoraire]*. 
...
.. admonition:: Règle de réification

  Soit :math:`A` une association entre les types d'entité
  :math:`\{E_1, E_2, \ldots, E_n\}`. La transformation de :math:`A` 
  en type d'entité :math:`E_A` s'effectue en deux étapes :

   * On attribue un identifiant autonome à :math:`E_A`.
   * On crée une association :math:`A_i` de type '(0..n):(1..1)' entre
     :math:`E_A`  et chacun des :math:`E_i`.  
     
   Chaque instance de :math:`E_A` doit impérativement être associée
   à une instance (et une seule) de chacun des :math:`E_i`. Cette contrainte
...
On obtient le schéma de la :numref:`seance`.

.. _seance:
.. figure:: ../figures/seance.png
   :width: 60%
   :align: center
...
.. note:: Cette partie présente des notions assez avancées qui sont introduites
...
est le *reportage*, ce qui donne donc le modèle de la  :numref:`heritage`.
...
.. figure:: ./figures-sql/heritage.png
   :width: 50%
   :align: center
...
.. eqt:: conc3-1
    ...
    Dans notre base des voyageurs, quelles sont d'après vous les entités faibles?
   
    A) :eqt:`C`  Les activités
    #) :eqt:`I`  Les séjours
    #) :eqt:`I` Aucune

.. eqt:: conc3-2

    Dans notre base des immeubles, quelle entité aurait-on pu modéliser comme "faible"?
   
    A) :eqt:`I`  Les immeubles
    #) :eqt:`C`  Les appartements
    #) :eqt:`I` Les propriétaires

.. eqt:: conc3-3

    Dans notre base des voyageurs, quelle entité réifiée aurait pu être modélisée
    avec une association ternaire?
   
    A) :eqt:`I`  Les activités
    #) :eqt:`C`  Les séjours
    #) :eqt:`I` Les voyageurs

.. eqt:: conc3-4

    Dans notre base des voyageurs, quelle entité vous semble la plus susceptible de donner
    lieu à une modélisation par spécialisation?
   
    A) :eqt:`I`  Les voyageurs, en créant une sous-classe par pays
    #) :eqt:`I`  Les séjours, en créant une sous-classe par période
    #) :eqt:`C` Les logements, en créant une sous-classe par type
...
Pour rappel, voici le schéma E/A de la base des films (:numref:`films2`), discuté 
précédemment.
...
.. important:: Si une association plusieurs-à-plusieurs n'a pas d'attribut propre,
   ...
   Exemple: la :numref:`vu-film` montre une association plusieurs-plusieurs
...
par exemple :math:`idFilm \to idArtiste`, sont en italiques pour indiquer
...
et modélisée dans le schéma E/A de la  :numref:`films2`, qu'un film
...
.. note:: Les valeurs des clés primaires et étrangères sont complètement indépendantes
...
.. note:: La spécialisation est une notion avancée dont la représentation en relationnel
...
Dans le même
esprit, voici la table *Reportage*.

.. list-table:: La table ``Reportage``
   :widths: 10 10
   :header-rows: 1
...
.. eqt:: conc4-1
    ...
    A) :eqt:`I`  C'est l'identifiant de l'entité ``A`` dans une association
       plusieurs-à-un ``A -> B``
    #) :eqt:`I` C'est l'identifiant d'une association
    #) :eqt:`C`  C'est l'identifiant de l'entité ``B`` dans une association
       plusieurs-à-un ``A -> B``
...
.. eqt:: conc4-2
    ...
    A) :eqt:`I`  Avec le même nom que la clé primaire référencée, pour permettre
       une jointure naturelle
    #) :eqt:`C` Le nommage est libre
    #) :eqt:`I` On lui donne le nom de l'association
...
.. eqt:: conc4-4
    ...
    Qu'est-ce qui caractérise la clé d'une relation issue d'une association 
    plusieurs-plusieurs
   
    A) :eqt:`C`  Chaque attribut de cette clé est également une clé étrangère
    #) :eqt:`I` La contrainte d'unicité s'applique à chaque attribut de cette clé
    #) :eqt:`I` Rien de spécial

.. eqt:: conc4-5

    Si j'avais choisi de réifier l'entité ``Rôle``, en lui donnant un identifiant
    propre, que devient la clé ``(idFilm, idActeur)``?
   
    A) :eqt:`I`  Elle disparaît
    #) :eqt:`I` Elle devient toujours une clé secondaire
    #) :eqt:`C` C'est un choix à faire par le concepteur
...
On obtient donc
le schéma de la :numref:`ea-immeubles`. 

.. _ea-immeubles:
.. figure:: ./figures-sql/schema-immeubles.png
   :width: 80%
   :align: center
...
à cet immeuble. Le schéma E/A devient alors celui de la :numref:`ea-immeubles-faible`.

.. _ea-immeubles-faible:
.. figure:: ./figures-sql/schema-immeubles-avec-faible.png
   :width: 80%
   :align: center
...
est donné par la :numref:`ea-immeubles-reifie`.

.. _ea-immeubles-reifie:
.. figure:: ./figures-sql/schema-immeubles-reifie.png
   :width: 80%
   :align: center
...
.. _Ex-conc-1:
.. admonition:: Exercice `Ex-conc-1`_: compréhension d'un schéma E/A

    .. _medical:
    .. figure:: ./figures-sql/medical.png
        :width: 60%
        :align: center

        Un schéma pour un centre médical

    On vous donne un schéma E/A (:numref:`medical`) représentant des
...
.. _Ex-conc-2:
.. admonition:: Exercice `Ex-conc-2`_: un quotidien

    Voici le schéma E/A (:numref:`quotidien`) du
    système d'information (très simplifié) d'un quotidien.

    .. _quotidien:
    .. figure:: ./figures-sql/quotidien.png
        :width: 60%
        :align: center

        Système d'information d'un quotidien
    ...
    .. ifconfig:: concexos in ('public')
      ...
      .. admonition:: Correction
         ...
         Réponses:
            ...
            - Le numéro d'un journal est une entité faible, ce qui semble censé: un numéro
              est indissociable du journal qui le publie.

.. _Ex-conc-3:
.. admonition:: Exercice `Ex-conc-3`_: cardinalités

    Voici (:numref:`media`) le début d'un schéma E/A pour la gestion d'une médiathèque. La
    ...
    .. _media:
    .. figure:: ./figures-sql/mediatheque.png
        :width: 90%
        :align: center

        Début de schéma pour une médiathèque

    Répondez aux questions suivantes
    
        - Complétez le modèle de la :numref:`media` en ajoutant les cardinalités.
        ...
        - Transformez l'association "Joue" dans la :numref:`media`  en entité
    ...
    .. ifconfig:: concexos in ('public')
      ...
      .. admonition:: Correction
         ...
         La :numref:`media-sol` montre la première étape avec les cardinalités
         et l'instrument. Ma solution présente ce qui semble le plus vraisonnable.
         Vous avez trouvé autre chose? Si c'est défendable, personne
         ne peut dire que vous avez tort, à condition que vous sachiez défendre et expliquer vos choix.
         
         .. _media-sol:
         .. figure:: ./figures-sql/mediatheque-solution.png
            :width: 90%
            :align: center

            Première étape de la solution

         La :numref:`media-sol2` donne la solution complète. La nouvelle entité Contribution
         est liée à un interprète et une plage, ce qui préserve la signification initiale de
         l'association. On a généralisé l'entité Interprète en Personne, ce qui présente
         de multiples avantages. Une personne peut être interprète, mais aussi auteur ou produtcteur.
         
         .. _media-sol2:
         .. figure:: ./figures-sql/mediatheque-sol2.png
            :width: 90%
            :align: center
...
.. _Ex-conc-4:
.. admonition:: Exercice `Ex-conc-4`_: normalisation

    Reprenons le schéma de la :numref:`medical`. Nous allons appliquer l'algorithme de normalisation.
    
    - Indiquez toutes les dépendances fonctionnelles définies par ce schéma
    - En déduire le schéma des relations, en mettant en valeur clés primaires et clés
      étrangères
    ...
    .. ifconfig:: concexos in ('public')
      ...
      .. admonition:: Correction
         ...
         un ordinateur saurait le faire).
           ...
           - :math:`codeMédicament \to libellé`
           - :math:`matricule \to nom`
           - :math:`noSS \to nom`
           - :math:`noConsultation \to date`
           - :math:`noConsultation \to matricule`
           - :math:`noConsultation \to noSS`
           - :math:`codeMédicament, noConsultation \to nbPrises`
...
.. _Ex-conc-5:
.. admonition:: Exercice `Ex-conc-5`_: schéma relationnel

    Donner le schéma relationnel pour le modèle de la médiathèque (partez du modèle
    initial, celui de la :numref:`media` ou, mieux, de votre modèle final.
...
.. _Ex-conc-6:
.. admonition:: Exercice `Ex-conc-6`_: rétroconception
    ...
    .. ifconfig:: concexos in ('public')
      ...
      .. admonition:: Correction
         ...
         Au départ, Séjour était probablement une association plusieurs-plusieurs. Notez
         que Activité est une entité faible, ce qui peut se déduire de la structure de son
         identifiant, constitué en partie de l'identifiant d'une autre entité.

         .. _schema-voyageurs-mieux:
         .. figure:: ./figures-sql/association-sejour.png
            :width: 60%
            :align: center

            Le schéma de la base Voyageur, montrant le Séjour comme une association

         Puis le concepteur s'est sans doute rendu compte qu'un même voyageur ne pouvait
         visiter un même logement qu'une seule fois, et a décidé de réifier l'association
         Séjour, ce qui lève cette contrainte.

         .. _association-sejour-reifiee:
         .. figure:: ./figures-sql/schema-voyageurs-mieux.png
            :width: 80%
            :align: center

            Réification de l'association Séjour

************************************************
Atelier: étude du cas "Zoo": le schéma normalisé
************************************************

Nous reprenons l'étude du schéma "Zoo" vu dans le chapitre :ref:`chap-modrel`.
Maintenant, on va chercher un schéma correct.
  ...
  - Pour conclure, vous pouvez (optionnel) installer un outil de conception comme
    ...
    (voir :numref:`mysqlworkbench`). À partir de là vous pouvez tout paramétrer et engendrer
    automatiquement le schéma de la base. 

    .. _mysqlworkbench:
    .. figure:: ./figures-sql/mysqlworkbench.png
       :width: 80%
       :align: center

       L'outil de conception de MySQL
   ...
   .. ifconfig:: concexos in ('public')
       ...
       .. admonition:: Correction
          ...
          Les clés primaires sont en gras. Il faudrait améliorer certaines choses, notamment
          en identifiant les gaerdiens autrement que par leur prénom... Les clés étrangères
          sont en italiques.
          ...
          Le schéma de la base est donné par la :numref:`schema-zoo`. Vous noterez
          qu'on ne représente pas dans le schéma les clés étrangères puisque
          qu'elle sont remplacées par les associations.
          
         .. _schema-zoo:
         .. figure:: ./figures-sql/schema-zoo.png
            :width: 90%
            :align: center

            Modéle de la base Zoo
